...
$question1=mysql_query("select question from login where user_name='".$custid."' AND user_password='".$pass."'");  
while($row2 = mysql_fetch_array($question1))
{
$question=$row2[0];
}

$type=mysql_query("select user_type from login where user_name='".$custid."' AND user_password='".$pass."' AND question='".$qns."' AND ans='".$ans."'");  
...
{ 
$result = mysql_query("delete from login where user_name='".$custid."' and user_password='".$pass."' and user_type='".$authtype."' AND question='".$qns."' AND ans='".$ans."'"); 
 
$db_close=mysql_close();			 

if($result )			    
{		
echo "Selected Record is Deleted...";
echo "<p>Click <a href='delete_acc.php'> here </a> to add another";
exit(0); //echo "Added To The Database";	
}			 
else			  
{			
echo "Selected Record is Not Deleted...";
echo "<p>Click <a href='delete_acc.php'> here </a> to go back";
exit(0);			  
} 		}}
...
<FONT face="Verdana, Arial, Helvetica, sans-serif" color=#ffffff size=2>| Update User </FONT>
</STRONG>
<STRONG>
<a href='delete_acc.php?mode=admin'>
<FONT face="Verdana, Arial, Helvetica, sans-serif" color=#aaeeff size=2>| Delete User </FONT>
</STRONG>
</TD>
</TR>  
...
<TD class=firstalt align=left><select  name="custid" AUTOCOMPLETE="ON" tabindex="1" >	
<?  
...
<div align="center"><p><input name="Submit" type="submit" class="admin_add_items" value="Delete">&#160;&#160;&#160;&#160;          
